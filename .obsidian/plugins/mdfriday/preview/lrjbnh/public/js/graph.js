...
(function() {
  ...
  async function initGraph() {
    ...
    console.log('[Graph] After initial processing:', slug);

    // ⭐ Key fix: Simplify slug BEFORE removing baseURL
    // This ensures "/public/index" becomes "public" and "/public/" also becomes "public"
    slug = simplifySlug(slug);
    console.log('[Graph] After first simplifySlug:', slug);
    ...
    if (baseURL) {
      if (slug.startsWith(baseURL + "/")) {
        // Normal case: baseURL is a prefix like "public/something"
        slug = slug.substring(baseURL.length + 1);
        console.log('[Graph] After removing baseURL prefix:', slug);
      } else if (slug === baseURL) {
        // Homepage case: "/public/" or "/public/index.html" both become "public"
        // This should map to "index"
        slug = "index";
        console.log('[Graph] Detected homepage, mapped to "index"');
      }
    } 
    ...
    // Final simplification to ensure consistent format
    slug = simplifySlug(slug);
